> **Status: L1 SHIPPED · L2 SHIPPED · L3 SHIPPED · §4.2 `BLOCK_BIND` SHIPPED** — Brainstorm-converged design (2026-05-17); reviewed and corrected 2026-05-20. PRs split L1 → L2 → L3 as three independent rollouts.
>
...
> - **L2 (Phase 2) — SHIPPED.** Shipped first: the `#inline` annotation (lexer `#` token + statement-position parsing) and recursion rejection (E240/E244/E246/E249). Shipped completing L2: eligible non-`#inline` user `fn`s called from ≥2 sites compile **once** into a subprogram block and dispatch at runtime via a real `BLOCK_CALL` opcode — body stored once in `ProgramSlot::instructions[]`, relieving program-size / i-cache pressure. **Design pivot from §4.2 — the swap-time `expand_block_calls()` model was abandoned.** Compile-time expansion produces audio bytecode byte-identical to per-site inlining (zero size/i-cache benefit — see §6.1); instead L2 reuses L3's already-shipped runtime subprogram table (`ProgramSlot::blocks[]`, the block-aware load handshake, `set_state_id_xor` isolation). `BLOCK_CALL` carries `block_id` in `inst.rate`, caller args in `inst.inputs[0..4]`, result in `inst.out_buffer`, a per-call-site disambiguator in `inst.state_id`; `BlockEntry` gained `param_base` + `body_output` (12 bytes). `RET` stays a reserved no-op — body length lives in `BlockEntry::length`. Eligibility = speculative-compile-with-rollback (cheap signature gate, then a one-shot body compile rolled back on any error / empty-or-oversize body / nested dispatch opcode); ineligible or non-dispatchable call sites (string/array/record/`_` args) fall back to inlining — a pure optimization. Per-call-site state isolation + hot-swap continuity via a `block:<fn>@callsite_<N>` path hash XOR-composed by the VM. Rollback knob: the `inline_all_fns` compiler option forces the pre-L2 inline-everywhere behavior. **`BLOCK_BIND` (§4.2) shipped as a follow-up (2026-05-20):** the >5-param slot path is now built — `fn`s with 6..`MAX_SHARED_FN_PARAMS` (32) parameters are shareable. Params 0..4 ride `BLOCK_CALL.inputs[0..4]`; params 5..N-1 ride a contiguous run of `BLOCK_BIND` opcodes (slot index in `rate`, source buffer in `inputs[0]`) emitted immediately before the `BLOCK_CALL`. The VM dispatch loop scans the run and copies the extra slots into the body's convention param bank. `fn`s beyond 32 params still fall back to inlining (graceful, never an error). **Dispatch-uniformity followup (2026-05-24, `d95aa8b`):** the L2 ship wired `BLOCK_CALL` / `BLOCK_BIND` / `SKIP_IF_*` / `LOOP_STATIC` into `execute_program`'s main IP loop but left every subprogram-body runner (`run_voice_pool`, `run_foreach_per_iteration`, `run_foreach_shared`, `run_event_map_closure`, `run_event_filter_closure`, `execute_block_call`'s own body loop) driving instructions through `VM::execute()` directly — whose switch has no case for the meta-opcodes, so any `BLOCK_CALL` inside a subprogram body silently no-op'd. A new `VM::execute_step(slot, body, ip) -> next_ip` helper owns the meta-opcode dispatch and is now the single entry point for both the main loop and every body runner; nested `BLOCK_CALL` (fn-in-fn, fn-in-event-map-closure, fn-in-voice-body) now dispatches correctly.
...
> **As shipped (2026-05-20).** `BLOCK_CALL` is a real runtime-dispatch opcode
> (the swap-time `expand_block_calls()` model was abandoned — see the Status
> block). `block_id` rides `inst.rate`; `RET` is a reserved no-op (body length
> lives in `BlockEntry::length`). `BLOCK_BIND` shipped per the row below — it is
> consumed by the VM dispatch loop, not by any expansion pass.

| Opcode | Encoding | Semantics |
|---|---|---|
| `BLOCK_CALL` | `inputs[0..4] = bound buffer indices for first 5 logical slots`; `out_buffer = output_buf`; `state_id = callsite_disambiguator`; `block_id` in `inst.rate` | Dispatch-loop opcode: invokes the shared `fn` body resident in `ProgramSlot::blocks[block_id]`, copying args into the body's convention param bank, running the body once, copying the result back |
| `BLOCK_BIND` | `rate = slot_index` (uint8, range 5..`MAX_SHARED_FN_PARAMS`-1); `inputs[0] = buf_idx`; `out_buffer = 0xFFFF`; emitted as a contiguous run immediately before `BLOCK_CALL` when a call site binds logical slots beyond `inputs[0..4]` | Binds one additional logical input slot to a buffer for the immediately following `BLOCK_CALL`. The VM dispatch loop scans the `BLOCK_BIND` run and copies each bound slot into the body's param bank. Multiple `BLOCK_BIND`s precede a single `BLOCK_CALL`; order doesn't matter (slot index is explicit) |
| `RET` | no operands | Reserved no-op terminating a subprogram body. Body length lives in `BlockEntry::length`; the VM never executes a `RET` |
...
3. For call-sites with more than 5 inputs (beyond the `inputs[0..4]` slot bank), emit a leading sequence of `BLOCK_BIND` opcodes — one per slot 5..N-1, each carrying `rate = slot_index` and `inputs[0] = buf_idx` — immediately before the `BLOCK_CALL`. The body reads slot K from the fixed convention param bank `[param_base, param_base + frame_slot_count)`. (Most user fns fit in 5; this is a rare path.) **As shipped:** the VM dispatch loop scans the `BLOCK_BIND` run and copies the extra slots into the param bank before running the body — there is no `expand_block_calls()` rewrite pass. Shareable `fn`s are capped at `MAX_SHARED_FN_PARAMS = 32` parameters (`akkado/src/codegen_functions.cpp`); beyond that the `fn` falls back to per-site inlining.
...
| `E248` | *(unused)* `BLOCK_BIND` slot index ≥ `frame.param_count` for the target block. **Not emitted as a diagnostic** — codegen owns both the `BLOCK_BIND` slot index and the block's `frame_slot_count`, so a mismatch is unreachable from valid source. The VM `execute_block_call` copy loop still guards `slot_idx < frame_slot_count` defensively (a stray bind is dropped, never scribbles outside the param bank). | — |
...
- New opcodes: `BLOCK_CALL`, `RET`, `BLOCK_BIND`. `BLOCK_CALL`/`RET` shipped
  with L2's runtime-dispatch model (commit `21f73ef`). `BLOCK_BIND` shipped as
  a 2026-05-20 follow-up — it carries call-site arguments for logical slots
  >=5, lifting the shareable-`fn` parameter cap from 5 to
  `MAX_SHARED_FN_PARAMS` (32). The VM dispatch loop scans a `BLOCK_BIND` run
  and copies the extra slots into the body's param bank.
