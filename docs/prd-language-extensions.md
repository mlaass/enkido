...
```akkado
// Basic chaining
sine(440).lp(800)                    // → lp(sine(440), 800)

// Multi-step chain
saw(440).lp(800).delay(0.3, 0.5, 1, 0.5)  // → delay(lp(saw(440), 800), 0.3, 0.5, 1, 0.5)

// With pipe
sine(440).lp(800) |> out(@, @)       // dot resolves before pipe
...
// Error: non-pure body
const fn bad(x) -> sine(x)          // ERROR: osc is not pure
...
```akkado
match(event) {
    {freq, vel}: sine(freq) * vel
    {freq, vel} if vel > 0.5: saw(freq) * vel
    _: 0.0
}
```
...
**`as` binding syntax:**
```akkado
n"c4 e4 g4" as {freq, vel, trig} |> sine(freq) |> @ * vel
```
...
Example compilation of `{freq, vel}: sine(freq) * vel`:
...
r = {freq: 440, vel: 0.8}
match(r) {
    {freq, vel}: sine(freq) * vel       // → sine(440) * 0.8
    _: 0
}

// As binding destructuring
n"c4 e4 g4" as {freq, vel, trig} |> sine(freq) |> @ * vel |> out(@, @)
...
- With type system: emit error `"slow() expects Pattern as first argument, got Signal"` if user writes `sine(440).slow(2)`
...
sine(440).slow(2)                    // ERROR: slow expects Pattern, got Signal
```
...
// Array default
const fn harmonics(f, n) -> range(1, n + 1) |> map(@, (i) -> f * i)
fn additive(freqs = harmonics(220, 4)) -> sum(map(freqs, (f) -> sine(f)))
...
match(vel) {
    0.0..0.3: sine(440) * 0.3
    0.3..0.7: sine(440) * 0.6
    0.7..1.0: saw(440) * 1.0
    _: dc(0)
}
...
sine(modified.freq) * modified.vel   // → sine(880) * 0.8
...
fn play(note = {..base_note, vel: 0.5}) -> sine(note.freq) * note.vel

// Range patterns + match destructuring
n"c4 e4 g4" as {freq, vel} |>
match(vel) {
    0.0..0.3: sine(freq) * vel
    0.3..0.7: tri(freq) * vel
    0.7..1.0: saw(freq) * vel
    _: dc(0)
} |> out(@, @)
```
