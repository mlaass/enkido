...
**Observed:** Playing repeated notes in quick succession on a live MIDI keyboard through `midi() |> lead(@.freq, @.gate, @.vel) |> out` produces a subtle pop on each retrigger. The chain is `saw(glide(freq, 0.05)) + sqr(glide(freq, 0.05)+0.5) |> lp |> @ * adsr(gate, 0.01, 0.15, 0.75, 0.35) * vel * amp`.
