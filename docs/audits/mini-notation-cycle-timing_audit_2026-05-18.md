> **Status: SUPERSEDED 2026-05-19.** The Strudel-cycle-fitting fix shipped in `5c1c0ca` and the `.slow()/.fast()` wraps in `25bc560` were reverted. The current model is "cycle = beat" with top-level mini-notation as per-cycle alternation (deliberate divergence from Strudel/Tidal). See the Unreleased CHANGELOG entry for the new canon.
>
> **Status (original): PRD** — Audit + remediation plan. Treat as the implementation contract for the fix. Drafted 2026-05-18.
>
