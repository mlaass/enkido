...
The 70+ vintage drum machines of [`geikha/tidal-drum-machines`](https://github.com/geikha/tidal-drum-machines)
(TR-808/909/606, LinnDrum, MPC60, Oberheim DMX, …) are available as a
**built-in catalog** — no `samples()` directive and no UI step. Naming a
machine with `.bank()` auto-resolves it:
...
- A committed catalog (`web/static/samples/tidal-drum-machines/catalog.json`,
  generated by `scripts/generate-tdm-catalog.sh`) maps every machine's drum
  names to files, pinned to an upstream commit SHA.
- The audio is **never bundled** — each sample streams from
  `raw.githubusercontent.com` on first use and is cached (IndexedDB on the
  web, the native file cache for the CLI).
- A failed fetch is a **soft failure**: a warning is logged, that one voice
  is silent, and the rest of the program keeps playing.
- An unknown machine name falls back to the normal unknown-bank behavior.

Refresh the catalog (adopt new upstream content) by re-running
`scripts/generate-tdm-catalog.sh` and committing the updated `catalog.json`.

For offline CLI use, pre-download machines into the local cache:

```bash
scripts/download-tdm.sh RolandTR808 RolandTR909 LinnDrum   # selected
scripts/download-tdm.sh --all                              # everything
