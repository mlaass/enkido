...
> **Status: COMPLETE** — Surfaces the chord-note array carried inside every pattern event (`OutputEvent.values[]`) as first-class dynamic arrays in Akkado, so userspace closures can write arpeggiators, harmonizers, riff machines, and other polyphonic transforms without new C++ opcodes per use case. Removes the unused `ChordLit` (`C4'`) syntax. All four phases shipped 2026-05-21. Implementation notes vs. this PRD: (1) a chord surfaces either as one multi-value event (chord symbols) *or* as several single-value events sharing an onset (polyrhythm `[c4,e4,g4]`) — `SEQPAT_VALUES` and the `SEQPAT_FIELD` num_values selector both gather events at the active onset, so both forms work; (2) `notes`/`freqs` are NOT reserved identifiers (they are common variable names — they behave like `len`/`map`: special-cased but shadowable); (3) `e.notes` is wired as a pattern field accessor directly, since UFCS covers only method calls, not bare field access; (4) the Python `test_op_seqpat_values.py` experiment was dropped — there is no Python API to populate `SequenceState` events, so `cedar/tests/test_seqpat_values.cpp` covers the opcode with synthetic chord sequences instead.
